...
module "aks_projekt_active_directory_group" {
...
module "aks_projekt_subsciption_identity_ad_group_assignment" {
  ...
  principal_id         = module.aks_projekt_active_directory_group[each.key].object_id
}

# Create Resource Group
module "aks_projekt_resource_group" {
...
module "aks_projekt_virtual_network_subNet" {
  ...
  resource_group_name  = module.aks_projekt_resource_group.name
  virtual_network_name = module.aks_projekt_virtual_network.name
}

# Create Cloudflare Api Token for External DNS
module "aks_projekt_cloudflare_api_token" {
...
# Create AKS cluster
module "aks_projekt_aks_cluster" {
  ...
  default_node_pool_blocks = [
    {
      ...
      vnet_subnet_id              = module.aks_projekt_virtual_network_subNet["aks"].id
  ...
  dns_prefix                = module.aks_projekt_resource_group.name
  resource_group_name       = module.aks_projekt_resource_group.name
  location                  = module.aks_projekt_resource_group.location
  ...
  azure_active_directory_role_based_access_control_block = {
    ...
    admin_group_object_ids = [module.aks_projekt_active_directory_group["admin-group"].object_id]
  }
}

# Download kubeconfig file to local directory
module "aks_projekt_aks_cluster_kubeconfig" {
  ...
  file_content         = module.aks_projekt_aks_cluster.kube_config_raw
}

# Assign RBAC subscription level role to the deployment user
module "aks_projekt_subsciption_identity_assignment" {
  source               = "git@github.com:thanos1983/terraform//Azure/modules/RoleAssignment"
...
module "aks_projekt_storage_account" {
  ...
  network_rules_block = {
    ...
    default_action = "Deny"
    ip_rules = [] # module.aks_projekt_public_ips["aks"].ip_address
    virtual_network_subnet_ids = [module.aks_projekt_virtual_network_subNet["aks"].id]
  }
...
module "aks_projekt_knative_dns_records" {
...
module "aks_projekt_create_namespaces" {
  ...
  extra_vars = {
    ...
    client_cert = base64decode(module.aks_projekt_aks_cluster.kube_admin_config[0].client_certificate)
    ca_cert = base64decode(module.aks_projekt_aks_cluster.kube_admin_config[0].cluster_ca_certificate)
  }
}

# Create all roles bindings for the needed role(s)
module "aks_projekt_create_namespaces_roles_binding" {
  ...
  subject_blocks = [
    {
      kind      = "Group"
      namespace = each.key
      name      = module.aks_projekt_active_directory_group[each.key].object_id
    }
  ]
  depends_on = [
    module.aks_projekt_create_namespaces
...
# Create Secret for OpenAI in desired namespace(s)
module "aks_projekt_k8s_openai_primary_key" {
  source = "git@github.com:thanos1983/terraform//Kubernetes/modules/KubernetesSecretV1"
  for_each = toset(split(",", var.reflection-allowed-namespaces))
  metadata_block = {
    name      = "openai"
    namespace = each.key
  }
  data = {
    (var.demoOpenAiReferenceKey) = data.azurerm_cognitive_account.openai.primary_access_key
  }
  depends_on = [
    module.aks_projekt_create_namespaces
  ]
...
module "aks_projekt_cloudflare_k8s_dns_secret" {
  source = "git@github.com:thanos1983/terraform//Kubernetes/modules/KubernetesSecretV1"
  metadata_block = {
  ...
  data = {
    (var.cloudflare_secretKeyRef_key) = module.aks_projekt_cloudflare_api_token.value
  }
  depends_on = [
    module.aks_projekt_create_namespaces
  ]
...
module "aks_projekt_aks_cluster_helm_deployment_dependencies" {
...
output "test" {
  value = nonsensitive(module.aks_projekt_cloudflare_api_token.value)
  sensitive = false
}

# Create Cloudflare Secret in ISTIO GW namespace
module "aks_projekt_cloudflare_k8s_dns_secret_token" {
  source = "git@github.com:thanos1983/terraform//Kubernetes/modules/KubernetesSecretV1"
  metadata_block = {
    name      = var.cloudflare_secretKeyRef_name
...
module "aks_projekt_k8s_cert_manager_issuer_manifest" {
...
module "aks_projekt_aks_cluster_helm_deployment" {
  source            = "git@github.com:thanos1983/terraform//Helm/modules/Release"
  ...
  depends_on = [
    ...
    module.aks_projekt_cloudflare_k8s_dns_secret,
    module.aks_projekt_k8s_cert_manager_issuer_manifest,
    # module.aks_projekt_deploy_knative_operator_manifests,
  ]
}

# Download Knative Manifests
module "aks_projekt_download_knative_manifests" {
...
# Create GW Routes
module "aks_projekt_k8s_istio_gw_routes" {
  source    = "git@github.com:thanos1983/terraform//Kubernetes/modules/KubectlManifest"
...
module "aks_projekt_k8s_annotate_secrets" {
